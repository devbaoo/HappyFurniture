...
**Body:** (giống Create, tất cả field đều bắt buộc trong Update)
```json
{
  "name": "Knock-down",
  "code": "KD",
...
Body tương tự Create (thêm các field EN).
...
| `detail` | `detailEn` | Chi tiết (HTML) |
| `deliveryInfo` | `deliveryInfoEn` | Thông tin giao hàng |

#### Response Product (có đủ field i18n)
...
function getLocalizedField(
  obj: { name?: string; nameEn?: string | null },
  locale: 'vi' | 'en'
): string {
  if (locale === 'en' && obj.nameEn) return obj.nameEn;
  return obj.name ?? '';
}

// Ví dụ dùng
const productName = getLocalizedField(product, currentLocale);
const categoryName = getLocalizedField(category, currentLocale);
```

---

## 5. Tổng quan tất cả endpoints

### Assembly

| Method | Endpoint | Auth | Mô tả |
|--------|----------|------|-------|
| GET | `/api/assemblies` | ❌ | Danh sách (paged) |
| GET | `/api/assemblies/active` | ❌ | Tất cả active (cho dropdown) |
...
### Category (thêm fields mới)

| Field mới | Kiểu | Nullable |
|-----------|------|----------|
| `nameEn` | string | ✅ |
| `description` | string | ✅ |
| `descriptionEn` | string | ✅ |

### Product (thêm fields mới)
...
| `assemblyId` | int | ✅ |
| `assembly` | object | ✅ (nested trong response) |

### Product Filter (thêm param mới)

```
GET /api/products?assemblyId=1&categoryId=2&isActive=true&pageNumber=1&pageSize=12
```

| Param mới | Type | Mô tả |
|-----------|------|--------|
