...
const Header = () => {
    ...
    const placeholderClass = isDark ? "placeholder:text-white/30" : "placeholder:text-muted/60";
    const navBorder = isDark ? "border-white/10" : "border-border";
    const iconColor = isDark ? "text-white hover:text-white/60" : "text-primary hover:text-muted";
    const searchIconColor = isDark ? "text-white/40" : "text-muted";

    return (
        <header className={`absolute top-0 left-0 w-full z-50 ${isDark ? "" : "bg-white"}`}>
            ...
            <div className="mx-auto max-w-[1800px] px-10 w-full">
                <div className="flex items-center justify-between py-6 relative">
                    ...
                    <div className="w-[260px]">
                        <div className={`flex items-center border px-4 py-2 gap-2 ${isDark ? "border-white/50" : "border-stone-300"}`}>
                            <svg className={`w-4 h-4 ${isDark ? "text-white/70" : "text-stone-400"}`} viewBox="0 0 24 24" fill="none" stroke="currentColor">
                    ...
                    <div className="absolute left-1/2 -translate-x-1/2 text-center">
                        <div className={`flex items-center gap-2 ${isDark ? "text-white" : "text-stone-800"}`}>
                            ...
                            <div className="flex flex-col text-left">
                                ...
                                <span className={`text-[9px] ${isDark ? "text-white/70" : "text-stone-400"}`}>Make life more convenient</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="flex items-center gap-2">
                        {/* Vietnam flag */}
                        <button aria-label="Tiếng Việt" className="overflow-hidden rounded-sm hover:opacity-80 transition-opacity" style={{ width: 28, height: 20 }}>
                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 28 20" width="28" height="20">
                                <rect width="28" height="20" fill="#DA251D" />
                                <polygon points="14,4 15.76,9.42 21.41,9.42 16.83,12.7 18.58,18.12 14,14.84 9.42,18.12 11.17,12.7 6.59,9.42 12.24,9.42" fill="#FFFF00" />
                            </svg>
                        </button>
                        {/* US flag */}
                        <button aria-label="English" className="overflow-hidden rounded-sm hover:opacity-80 transition-opacity" style={{ width: 28, height: 20 }}>
                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 28 20" width="28" height="20">
                                ...
                                {[1.1, 3.3, 5.5, 7.7, 9.9].map((x, i) => (
                                    <g key={i}>
                                        {[1, 3, 5, 7, 9].map((y, j) => (
                                            (i + j) % 2 === 0 && <circle key={j} cx={x} cy={y * 0.95} r="0.55" fill="#fff" />
                                        ))}
                                    </g>
                                ))}
                            </svg>
            ...
            {/* Short decorative line + breadcrumb — only on non-home pages */}
            {!isDark && (
                <div className="mx-auto max-w-[1800px] px-10 w-full pb-3">
                    ...
                    <div className="flex items-center gap-2 text-[10px] tracking-[0.18em] uppercase text-stone-400">
                        ...
                        <span className="text-stone-600">
                            {pageLabels[pathname] ||
                                (pathname.startsWith("/product/") ? "Product" : pathname.replace("/", ""))}
                        </span>
                    </div>
                </div>
            )}
